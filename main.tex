...
\begin{frame}{Inside a U-Net Diffusion Model}
    \centering
    \begin{tikzpicture}[
            font=\scriptsize,
        ...
        \path[use as bounding box] (-1.2,-3.15) rectangle (12.7,2.55);
        ...
        % zoom on one cross-attention layer
        \node[visible on=<3->,draw=xattn,thick,rounded corners=1pt,
            minimum width=0.78cm,minimum height=0.32cm,inner sep=0pt] (sel) at (3.6,0.56) {};
        \node[visible on=<3->,draw=xattn,fill=xattn!6,thick,rounded corners=2pt,
            text width=6cm,align=left,inner sep=5pt] (zoom) at (3.2,-2.3)
        {\textbf{\textcolor{xattn}{Cross-attention}}\\
            $Q$ from the image latent, $K,V$ from the text $c$\\
            \textcolor{xattn}{the only path text takes into the model}};
        ...
        % the questions
        \node[visible on=<4->,text width=5cm,align=left,font=\small] at (10.15,-1.95)
        {\textcolor{gray!45!black}{\scriptsize$\sim$865M parameters, dozens of layers.}\\[0.6em]
